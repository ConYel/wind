...
nc_RNA_categories <- plyranges::read_gff2("../genome_transc_human/ncRNA_transcripts_100bp_RNA_Central_piRNAbank_hg38.gtf") %>% 
  as_tibble() %>% 
...
pdf(str_glue("{dat_path}/venn_diagram_DE_salmon_fC_limma__miR_450a_e.pdf"))
vennDiagram(results, 
    include=c("up", "down"),
    counts.col=c("red", "blue"),
    circle.col = c("red", "blue", "green3"))
...
map2(fc_DES, salmon_DES, ~.x %>% 
  ...
  write_tsv(str_glue("{str_remove(dat_path[1],'/salmon|/featureCounts')}/all_comparisons_voom_QW_TMM_salmon_fc_LFCs_{todate}.txt"))
...
all_comp <- read_tsv(str_glue("{str_remove(dat_path[1],'/salmon|/featureCounts')}/all_comparisons_voom_QW_TMM_salmon_fc_LFCs_{todate}.txt"))

smallRNAs_gtf <- plyranges::read_gff2("../genome_transc_human/ncRNA_transcripts_100bp_RNA_Central_piRNAbank_hg38.gtf") %>% 
  as_tibble() %>% 
  select(gene_id, gene_type) %>% 
  distinct(gene_id, .keep_all = TRUE) %>% 
...
all_comp %>% 
  left_join(RNACentral_ids) %>% 
   write_tsv(str_glue("{str_remove(dat_path[1],'/salmon|/featureCounts')}/all_comparisons_with_gene_names.txt"))
...
# add the Annotation ----
#expression
ht_df <- data.frame(samples = FC_mat_1 %>% colnames,
                batch =  fc_list$samples$batch,
                row.names = FC_mat_1 %>% colnames)
ha_1 <- HeatmapAnnotation(df = ht_df,
                          annotation_name_side = "left",
                          col = list(samples = fc_list$colours %>% 
                                       set_names(FC_mat_1 %>% 
                                                   colnames),
                                     batch =  RColorBrewer::brewer.pal(n = 9, name = "Set3") %>% 
                                       rep(each = 2) %>% 
                                       as_factor() %>% 
                                       set_names(fc_list$samples$batch)))

# lFCS
ha_1_LFCs <- HeatmapAnnotation(Tumor_vs_Normal = c("FeatureCounts", "salmon"),
                              col = list(Tumor_vs_Normal = wes_palettes$Moonrise2[1:2] %>%
...
draw(ht_1+ht_1_lFCs,column_title = str_glue("Heatmap of {nrow(FC_mat_1)} DE piRNAs"),merge_legend = TRUE)

pdf("BRCA_tumour_vs_normal_heatmap_FC.pdf")
draw(ht_1+ht_1_lFCs,
     column_title = str_glue("Heatmap of {nrow(FC_mat_1)} DE piRNAs"), merge_legend = TRUE)
dev.off()
```
