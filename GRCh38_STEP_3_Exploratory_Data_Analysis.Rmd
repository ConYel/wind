...
Here we show both ways although we need to manipulate some columns in the
...
dev.off()
```
## 18. Histograms of reads' length and general information  
### i. extract the information of length for reads
Get the reads from FeatureCounts resulted bam and collapse them per length and count.

```{bash Reads info}
# note that pwd is the dir of wind/
...
###### -----------
## if it want to make only for reads aligning to piRNA uniquely or multimapped
...
###### ------------
## if we wanted only reads of of 15-49 bases
...
### ii. import the files with reads length
Return to the Rstudio docker in order to perform the visualization
```{r objects per method of reads length}
library(tidyverse)
library(plyranges)

##### piRNAs_hist <- read_tsv("piRNAs_hist.txt")

# a function for quantile summarization
...
# here we use the complete_biotypes_seqs object from paragraph 15.
complete_biotypes_seqs %>% select("smallRNA" = sncRNA, gene_type)
# 1 read per entry

## salmon ----
hist_all_RNA_sal <- list.files(
  path = file.path("Datasets_analysis", "GSE68246", "quants"),#find the files
  pattern = "hist_allRNA.txt",
  full.names = TRUE) %>% 
  vroom::vroom(id = "file", # import them with vroom
               col_names = c("read_count", "read_length", "smallRNA")) %>% 
  mutate(file = basename(file) %>% str_remove_all(".bam|_hist_all.+"),
         method = "salmon")
## featurecounts ----
hist_all_RNA_fc <- list.files(
  path = file.path("Datasets_analysis", "GSE68246", "star"),  
  pattern = "hist.txt", 
  full.names = TRUE) %>% 
  vroom::vroom(id = "file", 
               col_names = c("read_count", "read_length", "smallRNA")) %>% 
  # add information regarding multimapping reads and clean filenames
  mutate(file = basename(file) %>% str_remove_all("_Aligned.+"),
...
hist_all_RNA <- bind_rows(hist_all_RNA_sal, hist_all_RNA_fc) %>% 
...
### check range of length for each sample per method
hist_all_RNA %>% group_by(method, file) %>% 
  summarise(my_quantile(read_length)) 
### check how many reads per method and sample have been aligned
hist_all_RNA %>% group_by(method, file) %>% summarise(all_reads = sum(read_count))
```
### iii. make histograms

```{r make histograms per sample}
biotypes_forhist <- hist_all_RNA %>% 
  filter(gene_type != "NA") %>% 
  dplyr::count(gene_type) %>% 
  .$gene_type %>% 
  set_names() 

my_hist_plot <- biotypes_forhist %>% 
  map(~ hist_all_RNA %>%
        filter(Alignment != "Not_assigned", # remove not aligned reads
               gene_type == .x
               ) %>% 
        ggplot() +
        geom_bar(mapping =
        ...
        theme_bw() +
        ggtitle(.x) +
        facet_wrap(method~file, ncol = 3) +
        theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
       )

pdf(file.path(dirname(dat_path[1]), "salmon_FC_reads_length_grouped.pdf"))
my_hist_plot
dev.off()
