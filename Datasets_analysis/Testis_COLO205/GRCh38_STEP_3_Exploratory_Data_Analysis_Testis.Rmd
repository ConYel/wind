...
# keep only the non treated 
targets_file <- targets_file %>% filter(!str_detect(group, "dil"), 
                        !str_detect(run_accession,"431|432|433"))
...
function_PCA <- function(dgl_norm_data, plot_path, norm_method = "TMM", ntop = 500){
  ...
  print("There is no batch we use only the groups")
...
## 18. plots and tables regarding piRNAs and spike-ins
### i. Use two different datasets (1 testis, 1 COLO205)
```{r Dataset COLO205 FC, salmon}
# Featurecounts ----------------------------------------------------------------
# divide the primary dataset
dgl_COLO_fc <- DGEList(counts = fc$counts[, str_detect(colnames(fc$counts), "COLO")],
               samples =  targets_file %>% filter(str_detect(cells, "COLO")))

group_COLO_fc <- filter(targets_file, str_detect(cells, "COLO"))$group %>% droplevels()
#design matrix
design_COLO <- model.matrix(~0 + group_COLO_fc)

colnames(design_COLO) <- colnames(design_COLO) %>% 
  str_remove("..+_fc") 

rownames(design_COLO) <- filter(targets_file,str_detect(cells, "COLO"))$sample_name
#filtering
keep.exprs_COLO <- filterByExpr.DGEList(dgl_COLO_fc, design = design_COLO)
dgl_COLO_fc_filt <- dgl_COLO_fc[keep.exprs_COLO,,keep.lib.sizes=FALSE]

#normalization
cpm_COLO_fc_norm <- dgl_COLO_fc_filt %>% 
...
# Salmon -----------------------------------------------------------------------
# import only the COLO files
files_salm_COLO <- files_salm[names(files_salm) %in% as.character(dgl_COLO_fc_filt$samples$run_accession)]

# tximport
txi <- tximport::tximport(files_salm_COLO, type = "salmon",
  txOut = TRUE, countsFromAbundance = "lengthScaledTPM")
# DGElist
# from https://bioconductor.org/packages/release/bioc/vignettes/tximport/inst/doc/tximport.html
# we follow the instructions to import for edgeR 
...
# change the colnames of the salmon objects
identical(as.character(dgl_COLO_fc_filt$samples$run_accession),colnames(cts))
colnames(cts) <- dgl_COLO_fc_filt$samples$sample_name
colnames(normMat) <- dgl_COLO_fc_filt$samples$sample_name
...
# Creating a DGEList object for use in edgeR.
dgl_salmon_COLO <- DGEList(cts, 
                           samples = targets_file %>% filter(str_detect(cells, "COLO"))) %>% 
  scaleOffset(normMat) 
# remove objects.
rm(cts, normCts, normMat, txi)

keep.exprs_COLO_sal <- filterByExpr.DGEList(dgl_salmon_COLO, design = design_COLO)
dgl_salmon_COLO_filt <- dgl_salmon_COLO[keep.exprs_COLO_sal,,keep.lib.sizes=FALSE]

#normalization
cpm_salmon_COLO_norm <- dgl_salmon_COLO_filt %>% 
  cpm(normalized.lib.sizes = TRUE,
      log = FALSE,
      prior.count = 2) %>% 
  as_tibble(rownames = "sncRNA") 
...
CPM_COLO_fc_salmon <- bind_rows("FeatureCounts" = cpm_COLO_fc_norm,
                                "salmon" = cpm_salmon_COLO_norm,
                                .id = "method") 
```
### ii. Load the samples of testis

```{r Dataset Testis FC, salmon}
# Featurecounts ----------------------------------------------------------------
# divide the primary FC dataset
dgl_testis_fc <- DGEList(counts = fc$counts[, str_detect(colnames(fc$counts), "Testis|testis")],
               samples =  targets_file %>% filter(str_detect(cells, "Testis|testis")))

group_testis_fc <- filter(targets_file, str_detect(cells, "Testis|testis"))$group %>% droplevels()
#design matrix
design_testis <- model.matrix(~0 + group_testis_fc)

colnames(design_testis) <- colnames(design_testis) %>% 
  str_remove("..+_fc") 

rownames(design_testis) <- filter(targets_file,str_detect(cells, "Testis|testis"))$sample_name
#filtering
keep.exprs_testis <- filterByExpr.DGEList(dgl_testis_fc, design = design_testis)
dgl_testis_fc_filt <- dgl_testis_fc[keep.exprs_testis,,keep.lib.sizes=FALSE]

#normalization
cpm_testis_fc_norm <- dgl_testis_fc_filt %>% 
  cpm(normalized.lib.sizes = TRUE,
      log = FALSE,
      prior.count = 2) %>% 
  as_tibble(rownames = "sncRNA") 
...
# Salmon -----------------------------------------------------------------------
# import only the testis files
files_salm_testis <- files_salm[names(files_salm) %in% as.character(dgl_testis_fc_filt$samples$run_accession)]

# tximport
txi <- tximport::tximport(files_salm_testis, type = "salmon",
  txOut = TRUE, countsFromAbundance = "lengthScaledTPM")
# DGElist
# from https://bioconductor.org/packages/release/bioc/vignettes/tximport/inst/doc/tximport.html
# we follow the instructions to import for edgeR 
...
# change the colnames of the salmon objects
identical(as.character(dgl_testis_fc_filt$samples$run_accession),colnames(cts))
colnames(cts) <- dgl_testis_fc_filt$samples$sample_name
colnames(normMat) <- dgl_testis_fc_filt$samples$sample_name
...
# Creating a DGEList object for use in edgeR.
dgl_salmon_testis <- DGEList(cts, 
                           samples = targets_file %>% filter(str_detect(cells, "Testis|testis"))) %>% 
  scaleOffset(normMat) 
# remove objects.
rm(cts, normCts, normMat, txi)

keep.exprs_testis_sal <- filterByExpr.DGEList(dgl_salmon_testis, design = design_testis)
dgl_salmon_testis_filt <- dgl_salmon_testis[keep.exprs_testis_sal,,keep.lib.sizes=FALSE]

#normalization
cpm_salmon_testis_norm <- dgl_salmon_testis_filt %>% 
  cpm(normalized.lib.sizes = TRUE,
      log = FALSE,
      prior.count = 2) %>% 
  as_tibble(rownames = "sncRNA") 
...
# join the two methods ---------------------------------------------------------
CPM_testis_fc_salmon <- bind_rows("FeatureCounts" = cpm_testis_fc_norm,
                                "salmon" = cpm_salmon_testis_norm,
                                .id = "method") 

```
### iii. Plot the cpm of spike-ins in COLO205 before and after treatment

```{r barplot of spike ins}
spikes_ins_plot <- full_join(CPM_COLO_fc_salmon , CPM_testis_fc_salmon, 
                             by = c("method","sncRNA")) %>% 
  ...
  scale_x_discrete(name = "piRNA-like ~ spike-ins", guide = guide_axis(angle = 45)) + 
  scale_y_continuous(name = "Counts per million", labels = scales::comma, breaks = scales::pretty_breaks(n = 10)) +
  theme_bw() +
  scale_fill_manual(values=c('#999999','#E69F00')) +
  facet_wrap(~ name, ncol = 3) 

zoom_spikes_ins_plot <-spikes_ins_plot + coord_cartesian(ylim = c(0, 3000)) 

pdf(file.path(dirname(dat_path[1]), "spike_ins_Testis_COLO205_treat_FC_salmon.pdf"))
spikes_ins_plot
zoom_spikes_ins_plot
dev.off()
```
### iv. BoxPlot of the cpm of piRNAs in COLO205 and Testis before and after treatment
...
piRNAs_COLO_testis <- full_join(CPM_COLO_fc_salmon , CPM_testis_fc_salmon, 
                             by = c("method","sncRNA")) %>% 
