...
con_mat <- makeContrasts(
  Spheroid_Mono = MCF_7_Sphe - MCF_7_Mono,
  levels = design)
...
nc_RNA_categories <- file.path("human_data","sncRNA_spike_ins_piRNBnk_RNACent_GRCh38_v34.gtf") %>% 
  plyranges::read_gff2() %>% 
  as_tibble() %>% 
...
smallRNAs_gtf <- file.path("human_data","sncRNA_spike_ins_piRNBnk_RNACent_GRCh38_v34.gtf") %>% 
  plyranges::read_gff2() %>% 
  as_tibble() %>% 
...
piRNAs_DE <- list.files(path = dirname(dat_path[1]), 
  ...
  write_tsv(file.path(dirname(dat_path[1]), 
                      str_c("all_logFC_voom_QW_DE_piRNA_",
    todate,".txt")))
