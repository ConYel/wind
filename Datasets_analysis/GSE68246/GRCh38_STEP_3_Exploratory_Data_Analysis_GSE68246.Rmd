...
targets_file <- file.path(my_basename,"downloaded_samples") %>% 
  ...
  select(read_count, run_accession, experiment_accession, #select only the columns with basic info
         sample_title, study_accession, experiment_alias) %>% 
  group_by(experiment_accession, sample_title, experiment_alias) %>% 
  summarise(read_count = sum(read_count)) %>% 
  dplyr::rename(sample_name = experiment_alias) %>% 
  mutate(group = sample_title %>% 
           str_replace_all("-| ","_") %>% 
           str_remove_all("_\\(R.+"),
         batch = sample_title %>% str_remove_all(".+ate |\\)"),
