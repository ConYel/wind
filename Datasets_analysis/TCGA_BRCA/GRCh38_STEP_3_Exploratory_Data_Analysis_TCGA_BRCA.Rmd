...
targets_file <- tibble(
  ...
  sample_new_id = str_c(str_remove(group,"ary.+|_T.+"),
                        "_",
                        str_remove(batch,"^[:alnum:]{2}"))
  ) 
```
...
colnames(cts) <- targets_file$sample_new_id
colnames(normMat) <- targets_file$sample_new_id
...
colnames(fc$counts) <- as.character(targets_file$sample_new_id)
colnames(fc$stat) <-c("Status", as.character(targets_file$sample_new_id))
...
rownames(design) <- targets_file$sample_new_id
...
rownames(design_2) <- targets_file$sample_new_id
```
