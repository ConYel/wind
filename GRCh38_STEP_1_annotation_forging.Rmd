...
To begin with, Human piRNA sequences were downloaded from piRNABank to enrich 
in piRNA sequences the gtf file, and small RNA genome coordinates(bed files) 
from RNAcentral have been acquired.
...
# GRCh38 annotation in order to exclude sequences of piRNA
# that are inside other sequences
wget ftp://ftp.ebi.ac.uk/pub/databases/gencode/Gencode_human/release_36/gencode.v36.primary_assembly.annotation.gtf.gz -O my_data/human_data/hg38/gencode.v36.primary_assembly.annotation.gtf.gz
```
### ii. Preprocessing of the piRNAbank file

The fasta file from piRNAbank has U character instead of T in the sequences, 
so changes have been made using [sed](https://www.gnu.org/software/sed/)
```{bash sed fasta}
...
Afterwards, alignment of piRNA sequences to the human genome 
utilizing STAR aligner has been performed.
...
gene_annot <- read_gff2("human_data/gencode.v36.primary_assembly.annotation.gtf.gz")

# how many GRs for each type are in the gencode annotation:
gene_annot %>% 
  as_tibble %>% 
  count(type, sort = T)#exon:1430132,CDS:790404...
# how many GRs for type per gene_type are in the gencode annotation:
gene_annot %>% 
  as_tibble %>% 
  count(type, gene_type, sort = T)

#filter gene_annot for only exon and CDS
gene_annot_exon_CDS <- gene_annot %>% filter(type %in% c("exon", "CDS"))

#filter exon and CDS for protein_coding and immunoglobulin gene
...
c_piRNAs_GR_prot_cod <- c_piBNK_RCent_GR %>% # GR = 155.143
  ...
  as_tibble() %>% 
  unite(col = "seq_piBNK", seqnames:strand, sep = "_") %>% 
  distinct(seq_piBNK, .keep_all = TRUE) # GR = 5.643 to be excluded

# exclude them from combined ranges
c_piBNK_RCent_GR_filtered <- c_piBNK_RCent_GR %>% 
  as_tibble() %>% 
  unite(col = "seq_piBNK", seqnames:strand, sep = "_") %>% 
  filter(!seq_piBNK %in% c_piRNAs_GR_prot_cod$seq_piBNK) %>% 
...
# miscellaneous piRNA would be categorized as those that overlap with sequences of categories of these 25 types(containing pseudogenes and smallRNAs) from GENCODE:
gene_annot_exon_CDS %>% 
...
misc_piRNAs_GR_filt <- c_piBNK_RCent_GR_filtered %>% 
  filter(gene_type == "piRNA") %>% # GR = 133.553
  find_overlaps_directed(gene_annot_misc_piRNA, maxgap = -1L, 
                         minoverlap = 1L, suffix = c("_piRNAs", "_genes")) %>% 
  as_tibble() %>% 
  unite(col = "seq_piBNK", seqnames:strand, sep = "_") %>% 
  distinct(seq_piBNK, .keep_all = TRUE) %>% # GR = 12.899 to be recategorized
...
# finding the "puzzling" piRNAs
puzzling_piRNAs <- complete_piBnk_RCent %>% 
  count(seq_id, gene_type, sort = T) %>% 
  filter(duplicated(seq_id))# 1.055 puzzling piRNA sequences from 27.963

# calculating the ratio of misc_piRNA to piRNA GRs and summarizing
complete_piBnk_RCent %>% 
  filter(seq_id %in% puzzling_piRNAs$seq_id) %>% # 60.829 GRs, 2.693 are misc_piRNA
  group_by(seq_id) %>% 
  count(gene_type) %>% 
  summarise(
...
# calculating which gene_type are the "puzzling" piRNAs are inside
explor_puzzling_piRNAs <- complete_piBnk_RCent %>% 
  filter(seq_id %in% puzzling_piRNAs$seq_id) %>% 
...
# search puzzling_piRNAs gene_types in GENCODE 
explor_puzzling_piRNAs %>% 
  filter(!is.na(gene_type_GENC)) %>% 
  count(gene_type_GENC, sort = TRUE) 

# search puzzling_piRNAs gene_types in RNACENTRAL 
explor_puzzling_piRNAs %>% 
