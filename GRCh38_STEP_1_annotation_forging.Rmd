...
# miscellaneous piRNA would be categorized looking at the overlap with sequences of categories of these 25 types(containing pseudogenes and smallRNAs):
gene_annot_exon_CDS %>% 
...
misc_piRNAs_GR_filt <- c_piBNK_RCent_GR_filtered %>% 
  ...
  as_tibble() %>% 
  unite(col = "seq_piBNK", seqnames:strand, sep = "_") %>% 
  distinct(seq_piBNK, .keep_all = TRUE) %>% # GR = 12.889 to be recategorized
  select(seq_piBNK, seq_id, gene_name, gene_id, transcript_id, tag) %>% 
  unite(col = "newseq_id",c(seq_id,gene_name), sep = "_Gname_") %>% 
  unite(col = "GENCODE_annot", gene_id:tag, sep = "_GNC_")

# misc_piRNAs
complete_piBnk_RCent <- c_piBNK_RCent_GR_filtered %>% 
  as_tibble() %>% 
  unite(col = "seq_piBNK", seqnames:strand, sep = "_") %>% 
  left_join(misc_piRNAs_GR_filt) %>% 
  mutate(gene_id = 
           case_when(
             is.na(newseq_id) ~ seq_id,
             TRUE ~ newseq_id),
         gene_type =
           case_when(
             is.na(newseq_id) ~ gene_type,
             TRUE ~ "misc_piRNA")
           ) %>% 
  select(seq_piBNK, gene_id, everything())
...
complete_piBnk_RCent %>% 
  ...
  filter(gene_type %in% c("piRNA","misc_piRNA")) %>% 
  arrange(seq_id) %>% 
  select(seq_piBNK:seq_id,newseq_id) %>% filter(seq_id =="hsa_piR_008764_DQ581894")
  count(seq_id,sort = T)
```
