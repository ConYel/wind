...
sRNA %>% 
  ...
  write_tsv(here("human_data", "RNACentral","Genome_ranges_Sequences_100_filter.txt"))
...
# length stats for each RNAcentral category
sRNA %>% 
  as_tibble() %>% 
  group_by(gene_type) %>% 
  summarise(my_quantile(width),
            GRs_number = n(), 
            Sequences = n_distinct(sRNA_id)) %>% 
  write_tsv(here("human_data", "RNACentral","Genome_ranges_Sequences_100_filter_stats.txt"))
```
...
```
#### RNAcentral. keep the all sequences except piRNA for later use 

```{r filter more than 100bp}
tr_sRNA_long <- sRNA %>% 
  filter(gene_type != "piRNA") %>% # remove piRNAs
  as_tibble() %>% 
  mutate(sRNA_id = str_remove(sRNA_id,"_9606")) %>% 
  as_granges() %>% 
  # keep info about the standard chromosomes
  ...
  as_tibble() %>% 
  unite(col = "seq_s",seqnames:strand, sep = "_") %>% 
  distinct(seq_s, .keep_all = TRUE) %>% # remove dupl GRs 
  separate(col = seq_s,
           into = c("seqnames","start","end","width","strand"),
           sep = "_", convert = T) %>% 
  as_granges()
...
Studies from Tosar et. al[1](https://pubmed.ncbi.nlm.nih.gov/33376191/) [2](https://pubmed.ncbi.nlm.nih.gov/30271890/) have demonstrated that some piRNAs
...
```
Following this strategy we will also find the GRs of piRNAs that are overlapping
with small RNAs from RNAcentral.
...
misc_piRNAs_GR_RCent_filt <- c_piBNK_RCent_GR_filtered %>% 
  ...
  find_overlaps_directed(tr_sRNA_long,
                         maxgap = -1L, 
                         minoverlap = 1L, 
  ...
  select(seq_piBNK, sRNA_id_Rcentral, gene_type_Rcentral) %>% 
  unite(col = "RNACent_annot", sRNA_id_Rcentral:gene_type_Rcentral, 
        sep = "_geneType_")
```
...
piRNAbank_rCentral_fasta %>% 
  Biostrings::writeXStringSet(here("human_data","strigent_sncRNA_piRNBnk_RNACent_GRCh38_v34.fa"))
...
gtf_piB_RCentr %>% 
  write_gff2(here("human_data","strigent_sncRNA_piRNBnk_RNACent_GRCh38_v34.gtf"))
