...
public_and_all_comp <- all_comp %>% 
  ...
  write_tsv(str_glue("{str_remove(dat_path[1],'/salmon|/featureCounts')}/public_and_all_comp_voom_QW_TMM_salmon_fc_LFCs_{todate}.txt"))
```
...
targets_DEs_keep <- all_comp %>% 
  ...
  write_tsv(str_glue("{str_remove(dat_path[1],'/salmon|/featureCounts')}/all_comp_voom_QW_DE_targets_predicted_{todate}.txt"))
```
...
piRNAs_DE <- list.files(pattern = "all_comparisons_voom_QW_TMM") %>% 
  ...
  write_tsv(str_glue("{str_remove(dat_path[1],'/salmon|/featureCounts')}/all_comp_voom_QW_DE_piRNA_{todate}.txt"))
...
pdf("GSE68246_spheroid_vs_mono_heatmap_FC.pdf")
draw(ht_1+ht_1_lFCs,
